...
func animation(direction: float):
	...
	#is on the ground.
	if velocity.x > (player_controller.MAX_SPEED * 0.01) or velocity.x < (-player_controller.MAX_SPEED * 0.01):
		if is_on_floor():
	...
	if velocity.y > player_controller.FALLING_POINT and not is_on_floor():
